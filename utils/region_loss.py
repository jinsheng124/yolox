...
class FocalWithLogitsLoss(nn.Module):
    ...
    def forward(self,preds,targets):
        ...
        preds = self.clip_by_tensor(preds, self.epsilon, 1.0 - self.epsilon)
...
class YOLOLoss(nn.Module):    
    def __init__(self, num_classes, strides=[8, 16, 32]):
        ...
        self.bce_angle = nn.BCEWithLogitsLoss(reduction="none")
        # self.bce_angle = FocalWithLogitsLoss(alpha=0.25,gamma=2)
    ...
    def get_output_and_grid(self, output, k, stride):
        ...
        # output[...,4]  = torch.sigmoid(output[...,4])*3.142-1.571
        #-----------------------#
        #------------------------#
        return output, grid

    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs,have_angle = 180):
        #-----------------------------------------------#
        #   [batch, n_anchors_all, 4]
        #-----------------------------------------------#
        bbox_preds  = outputs[:, :, :4]
        #角度
        angle_preds = outputs[:,:,4:4+have_angle]
        #-----------------------------------------------#
        #   [batch, n_anchors_all, 1]
        #-----------------------------------------------#
        obj_preds   = outputs[:, :, 4+have_angle:5+have_angle]
        #-----------------------------------------------#
        #   [batch, n_anchors_all, n_cls]
        #-----------------------------------------------#
        cls_preds   = outputs[:, :, 5+have_angle:]
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                #----------------------------------#
                gt_ba_per_image = labels[batch_idx][..., 4:5]
                #------------------------------------#
                gt_classes              = labels[batch_idx][..., 5]

                angle_preds_per_image = angle_preds[batch_idx]
                ...
                angle_target = gt_ba_per_image[matched_gt_inds]
        ...
        #---------------------#
        angle_preds_filter = angle_preds.view(-1,have_angle)[fg_masks]
        # loss_angle = torch.sin(torch.abs(angle_preds.view(-1,have_angle)[fg_masks],angle_target))
        ttheta = torch.zeros_like(angle_preds_filter)
        for idx in range(len(angle_targets)):  # idx start from 0 to len(ps)-1
            # 3个tensor组成的list (tensor_angle_list[i])  对每个步长网络生成对应的class tensor  tangle[i].shape=(num_i, 1)
            theta = angle_targets[idx]  # 取出第i个layer中的第idx个目标的角度数值  例如取值θ=90
        ...
        loss_angle = self.bce_angle(angle_preds_filter,ttheta).sum()
        #----------------------------------------------#
        loss_iou = loss_iou*5.0 + loss_angle*0.1
        ...
        return loss / num_fg,loss_obj.item()/ num_fg,loss_cls.item()/ num_fg,loss_iou.item()/ num_fg
